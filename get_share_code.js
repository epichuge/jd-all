              ...
              if (data.data.bizCode === 0) {
                ...
                $.taskVos.map((item) => {
                  if (item.taskType === 14) {
					...
					ddFacCode=ddFacCode.concat( "@" , item.assistTaskDetailVo.taskToken);
					console.log('【DDFACTORY_SHARECODES】'+ddFacCode );
                  }
                });
              }
            }
            ...
            if (safeGet(data)) {
              ...
              if (data["ret"] === 0) {
                ...
                if (data.factoryList && data.productionList) {
				  ...
				  dreamFacCode=dreamFacCode.concat( "@" , data.user.encryptPin);
				  console.log('【DREAM_FACTORY_SHARE_CODES】'+dreamFacCode );
                }
              } else {
            ...
            if (safeGet(data)) {
              ...
              if (data["ret"] === 0) {
                if (data.active) {
				  ...
				  jxncFacCode=jxncFacCode.concat( "@" , JSON.stringify(shareCodeJson));
				  console.log('【JXNC_SHARECODES】'+jxncFacCode );
                } else {
      ...
      try {
        if (err) {
          ...
          if (
			...
			petCode=petCode.concat( "@" , $.petInfo.shareCode);
			console.log('【PETSHARECODES】'+petCode );
          ...
          if (err) {
            ...
            if (safeGet(data)) {
              ...
              if ($.taskList.filter(item => !!item && item['taskId']=== 3) && $.taskList.filter(item => !!item && item['taskId']=== 3).length) {
				...
				zzCode=zzCode.concat( "@" , $.taskList.filter(item => !!item && item['taskId']=== 3)[0]['itemId']);
				console.log('【ZZSHARECODES】'+zzCode );
              }
            }
          }
        } catch (e) {
...
async function getPlantBean() {
  ...
  async function jdPlantBean() {
    ...
    if ($.plantBeanIndexResult.code === "0") {
	  ...
	  planCode=planCode.concat( "@" , $.myPlantUuid);
      console.log('【PLANSHARECODES】'+planCode );
    } else {
      console.log(
...
async function getJDFruit() {
  ...
  async function jdFruit() {
    ...
    if ($.farmInfo.farmUserPro) {
      ...
      ddCode=ddCode.concat( "@" , $.farmInfo.farmUserPro.shareCode);
      console.log('【FRUITSHARECODES】'+ddCode );
    } else {
      /*console.log(
        ...
        if (err) {
          ...
          if (safeGet(data)) {
            ...
            if (data.success && data.data && data.data.userInviteCode) {
			  ...
			  crazyJoyCode=crazyJoyCode.concat( "@" , data.data.userInviteCode);
			  console.log('【JOYSHARECODES】'+crazyJoyCode );
            }
          }
        }
      } catch (e) {
      ...
      $.post(url, async (err, resp, data) => {
        try {
          ...
          if (data.data.bizCode === 0) {
			...
			mangheCode=mangheCode.concat( "@" , invites && invites[0]['assistTaskDetailVo']['taskToken']);
			console.log('【MANGHESHARECODES】'+mangheCode );
          }
        } catch (e) {
          $.logErr(e, resp);
        } finally {
  ...
  return new Promise(async (resolve) => {
    $.get(taskUrl(`user/QueryUserInfo`), (err, resp, data) => {
      try {
		...
		cfdCode=cfdCode.concat( "@" , strMyShareId);
		console.log('【CFDSHARECODES】'+cfdCode );
      } catch (e) {
        $.logErr(e, resp);
      } finally {
        resolve();
        ...
        if (err) {
          ...
          if (safeGet(data)) {
            ...
            if(data.code===0 && data.data.result){
			  ...
			  cashCode=cashCode.concat( "@" , data.data.result.inviteCode);
			  console.log('【CASHSHARECODES】'+ cashCode );
            }
          }
        }
      } catch (e) {
